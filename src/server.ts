  ...
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        ...
        "For Gmail: use gmail_create_draft to leave a DRAFT in the user's Drafts folder — this server never sends email; the user reviews and sends drafts themselves. " +
        "Shared drives are included by default in all operations.",
    },
  );

  registerAllTools(server, clients.drive, clients.docs, clients.sheets, clients.slides, clients.calendar, clients.gmail);
  return server;
}
